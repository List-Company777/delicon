<!DOCTYPE html>
<html lang="ja">
<head><meta charset="UTF-8"><title>新人キャスト通知</title></head>
...
<div style="background:#252535;border-radius:12px;padding:24px;border:1px solid #3a3a4a">
  <h2 style="color:#d4a843;margin-top:0">✨ 新人キャストが登録されました</h2>
  <p>{{ $user->name }} 様</p>
  <p>新しいキャストが登録されました。</p>
  <div style="background:#1a1a2e;border-radius:8px;padding:16px;margin:16px 0">
    ...
    @if($cast->comment)<p style="margin:8px 0;font-size:13px;color:#c8c4bc">{{ Str::limit($cast->comment, 100) }}</p>@endif
  </div>
  <a href="{{ route('cast.show', $cast->id) }}/"
     style="display:inline-block;background:#d4a843;color:#1a1a2e;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold">
    プロフィールを見る
  </a>
  <p style="color:#6a6a7e;font-size:12px;margin-top:24px">
    新人通知を受け取りたくない場合は <a href="{{ route('user.settings') }}/" style="color:#d4a843">通知設定</a> から変更できます。
  </p>
</div>
</body></html>
