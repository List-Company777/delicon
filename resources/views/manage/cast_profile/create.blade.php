    ...
    <form action="{{ route('manage.cast-profile.store') }}/" method="POST" enctype="multipart/form-data"
        ...
        <div class="px-4 py-4 bg-gray-50 border-t border-gray-100 text-right">
            <button type="submit"
                    ...
                    class="text-white text-sm font-bold px-6 py-2 rounded-lg {{ ($limit !== null && $currentCount >= $limit) ? 'bg-gray-400 cursor-not-allowed' : 'bg-red-600 hover:bg-business-700' }} transition">
                登録する
            </button>
        </div>
    </form>
