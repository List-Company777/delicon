...
<div class="max-w-lg mx-auto py-8 px-4">
    ...
    {{-- メールアドレス変更 --}}
    <h2 class="text-lg font-bold text-gray-700 mb-3 mt-10">メールアドレス変更</h2>
    ...
    <form action="{{ route('manage.email.update') }}/" method="POST"
          class="bg-white rounded-xl shadow-sm overflow-hidden">
        @csrf
        @method('PATCH')
        <table class="w-full text-sm">
            <tr class="border-b border-gray-100">
                ...
                <td class="px-4 py-3 text-gray-600">{{ auth()->user()->email }}</td>
            </tr>
            <tr class="border-b border-gray-100">
                <th class="bg-gray-50 text-gray-500 font-normal text-left px-4 py-3 whitespace-nowrap">
                    新しいアドレス <span class="text-red-400">*</span>
                </th>
                <td class="px-4 py-3">
                    <input type="email" name="email" required autocomplete="email"
                           class="w-full border border-gray-300 rounded px-3 py-1.5 text-sm focus:outline-none focus:border-yellow-400 @error('email') border-red-400 @enderror">
                    <p class="text-xs text-gray-400 mt-1">変更後、新しいアドレスに確認メールが届きます</p>
                </td>
            </tr>
        </table>
        <div class="px-4 py-4 bg-gray-50 border-t border-gray-100 flex justify-end">
