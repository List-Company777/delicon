@extends('layouts.app')
@section('title', $job ? 'スタッフ求人の編集' : 'スタッフ求人の追加')
@section('content')
...
<div class="max-w-4xl mx-auto px-4 pb-12">
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ route('manage.staff.index') }}" class="text-sm text-gray-400 hover:text-gray-600">← スタッフ求人一覧</a>
        <h2 class="text-lg font-bold text-gray-800">{{ $job ? '求人を編集' : '求人を追加' }}</h2>
    </div>

    @if($job)
        <form action="{{ route('manage.staff.update', $job->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm overflow-hidden">
            @csrf @method('PUT')
    @else
        <form action="{{ route('manage.staff.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm overflow-hidden">
            @csrf
    @endif
        ...
        <div class="px-4 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
            <a href="{{ route('manage.staff.index') }}" class="text-sm text-gray-400 hover:text-gray-600">キャンセル</a>
