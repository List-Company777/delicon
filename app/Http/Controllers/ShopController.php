...
class ShopController extends Controller
{
    public function show(Request $request, int $id)
    {
        ...
        if (! $shop->hasBudget() && $shop->genre_id) {
            ...
            if ($shop->area_id) {
                $relatedShops = (clone $base)
                    ->where('area_id', $shop->area_id)
                    ->limit(4)
                    ->get($cols);
            }

            if ($relatedShops->count() < 4 && $shop->prefecture_id) {
                $additional = (clone $base)
                    ->whereNotIn('id', $relatedShops->pluck('id')->push($shop->id))
                    ->where('prefecture_id', $shop->prefecture_id)
                    ->limit(4 - $relatedShops->count())
                    ->get($cols);
                $relatedShops = $relatedShops->merge($additional);
            }
        }
